...
class TestHandlingSDKVersions:
    ...
    def test_with_no_dependencies(
        ...
        path_to_json = Path(__file__).parent.joinpath("data/simple_wf.json")

        with open(path_to_json, "r", encoding="utf-8") as f:
            wf = pydantic.TypeAdapter(WorkflowDef).validate_json(json.load(f))
    ...
    def test_with_multiple_dependencies(
        ...
        path_to_json = Path(__file__).parent.joinpath("data/wildcard.json")

        with open(path_to_json, "r", encoding="utf-8") as f:
            wildcard_workflow_def = json.load(f)
            wildcard_workflow_def = wildcard_workflow_def.replace(
                "<WILDCARD_NAME>", python_import
            )
            wildcard_workflow_def = wildcard_workflow_def.replace(
                "<WILDCARD_VERSION>", import_version
            )
            wf = pydantic.TypeAdapter(WorkflowDef).validate_json(wildcard_workflow_def)
        ...
        if expected_sdk_dependency:
            assert sorted(pip) == sorted(
                [f"{python_import}{import_version}"]
                + [f"orquestra-sdk=={expected_sdk_dependency}"]
            )
        else:
            assert sorted(pip) == sorted([f"{python_import}{import_version}"])
    ...
    class TestHandlesSDKDependency:
        ...
        def test_replaces_declared_sdk_dependency(
            ...
            # Given
            sdk_version = sdk_import.replace("orquestra-sdk", "").strip()
            import orquestra.workflow_shared.packaging._versions
            ...
            with open(path_to_json, "r", encoding="utf-8") as f:
                wildcard_workflow_def = json.load(f)
                wildcard_workflow_def = wildcard_workflow_def.replace(
                    "<WILDCARD_NAME>", "orquestra-sdk"
                )
                wildcard_workflow_def = wildcard_workflow_def.replace(
                    "<WILDCARD_VERSION>", sdk_version
                )
                wf = pydantic.TypeAdapter(WorkflowDef).validate_json(
                    wildcard_workflow_def
                )
            task_inv = [inv for inv in iter_invocations_topologically(wf)][0]
            imports = {
                id: _build_workflow._pip_string(imp) for id, imp in wf.imports.items()
