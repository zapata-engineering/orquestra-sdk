...
class TestAction:
    ...
    @staticmethod
    def test_show_warning_if_unset_filters_and_no_flags(capsys):
        """
        If the interactive and all flags are both unset and the is one or more unset
        filter, we should get a warning displayed to the user.
        """
        # Given
        # CLI inputs
        config = ["<config sentinel>"]
