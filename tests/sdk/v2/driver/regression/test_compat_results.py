...
class TestWorkflowResults:
    ...
    def mock_results(self, monkeypatch):
        def _inner(sdk_version, scenario=None):
            extra = "{}".format(
                "" if scenario is None else f"_{scenario}"
            )
            status_json = (
                DATA_PATH / f"{sdk_version}{extra}_status.json"
            ).read_text()
            ids_json = (
                DATA_PATH / f"{sdk_version}{extra}_ids.json"
            ).read_text()
    ...
    @pytest.mark.parametrize(
        "sdk_version, scenario, expected",
        [
            ("v0.46.0", "json", ([100, "json_string"],)),
            ("v0.46.0", "json-single", ([100],)),
            ("v0.46.0", "pickle", ((100, "pickled_string"),)),
            ("v0.46.0", "pickle-single", (("pickled_string",),)),
            ("v0.47.0", None, (100, 100)),
        ],
    )
    def test_multi_workflow_outputs(
        self, mock_results, runtime, workflow_run_id, sdk_version, scenario, expected
    ):
        mock_results(sdk_version, scenario)
