...
from orquestra.sdk.schema import logs
from orquestra.sdk.schema.ir import TaskInvocationId
from orquestra.sdk.schema.workflow_run import WorkflowRunId

from ._services import redirected_logs_dir
...
def iter_user_log_paths(ray_temp: Path) -> t.Iterator[Path]:
    return filter(logs.is_worker, _iter_logs_paths(ray_temp))


def iter_env_log_paths(ray_temp: Path) -> t.Iterator[Path]:
    return filter(logs.is_env_setup, _iter_logs_paths(ray_temp))
...
def iter_task_logs(
    ...
    with worker_file_path.open() as f:
        marker_context: t.Optional[logs.TaskStartMarker] = None
        collected_lines: t.List[str] = []

        for line in f:
            if (marker := logs.parse_line(line)) is not None:
                if isinstance(marker, logs.TaskStartMarker):
                    ...
                    collected_lines = []
                elif isinstance(marker, logs.TaskEndMarker):
                    if marker_context is not None:
                        yield CapturedLogLines(
                            collected_lines,
                            marker_context.wf_run_id,
...
class DirectLogReader:
    ...
    def _get_env_setup_lines(self) -> logs.LogOutput:
        ...
        return logs.LogOutput(out=[line.decode() for line in log_line_bytes], err=[])

    def _get_system_log_lines(self) -> logs.LogOutput:
        ...
        return logs.LogOutput(out=[], err=[system_warning])

    def _get_other_log_lines(self) -> logs.LogOutput:
        other_warning = (
            "WARNING: we don't parse uncategorized logs for the local runtime. "
            "The log files can be found in the directory "
            f"'{self._ray_temp}'"
        )
        return logs.LogOutput(out=[], err=[other_warning])

    def _get_legacy_task_logs(
        self,
        wf_run_id: WorkflowRunId,
        task_inv_id_allow_list: t.Optional[t.List[TaskInvocationId]] = None,
    ) -> t.Dict[TaskInvocationId, logs.LogOutput]:
        log_paths = iter_user_log_paths(self._ray_temp)
        logs_dict: t.Dict[TaskInvocationId, logs.LogAccumulator] = {}
        for log_path in log_paths:
            for logs_batch2, wf_run_id2, task_inv_id2 in iter_task_logs(log_path):
                ...
                stream = logs.LogStreamType.by_file(log_path)
                logs_dict.setdefault(
                    task_inv_id2, logs.LogAccumulator()
                ).add_lines_by_stream(stream, logs_batch2)

        return {
            invocation: logs.LogOutput(out=log_output.out, err=log_output.err)
    ...
    def _get_task_logs(
        ...
        for log_path in (wf_logs_dir / "task").iterdir():
            ...
            logs_dict.setdefault(
                log_file_task_inv_id, logs.LogAccumulator()
    ...
    def get_task_logs(
        ...
        return logs_dict.get(task_inv_id, logs.LogOutput(out=[], err=[]))

    def get_workflow_logs(self, wf_run_id: WorkflowRunId) -> logs.WorkflowLogs:
        try:
