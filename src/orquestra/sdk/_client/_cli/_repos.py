...
from orquestra.sdk._client import _config, _dates, loader
from orquestra.sdk._client._driver._client import DriverClient, ExternalUriProvider
from orquestra.sdk._client._jwt import check_jwt_without_signature_verification
from orquestra.sdk._client._logs._interfaces import LogOutput, WorkflowLogs
from orquestra.sdk._client.abc import ArtifactValue
...
class WorkflowRunRepo:
    def get_config_name_by_run_id(self, wf_run_id: WorkflowRunId) -> ConfigName:
        ...
        Raises:
            orquestra.sdk._client.exceptions.RuntimeQuerySummaryError: when it wasn't possible
                to infer the runtime matching ``wf_run_id``.
            orquestra.sdk._client.exceptions.WorkflowRunNotFoundError: when it wasn't possible
                to fetch run details.
            orquestra.sdk._client.exceptions.UnauthorizedError: when authorization with the
                remote runtime failed when getting the run details.
            orquestra.sdk._client.exceptions.ConfigFileNotFoundError: when the config file
                couldn't be read.
            orquestra.sdk._client.exceptions.ConfigNameNotFoundError: when there's no
    ...
    def list_wf_run_summaries(
        ...
        Raises:
            ...
            orquestra.sdk._client.exceptions.UnauthorizedError: when connection with runtime
    ...
    def get_wf_by_run_id(
        ...
        Raises:
            orquestra.sdk._client.exceptions.WorkflowRunNotFoundError: when the wf_run_id
                doesn't match any available run ID.
            orquestra.sdk._client.exceptions.ConfigNameNotFoundError: when the named config is
    ...
    def get_task_run_id(
        ...
        Raises:
            ...
            orquestra.sdk._client.exceptions.ConfigNameNotFoundError: when the named config is
                not found in the file.
            orquestra.sdk._client.exceptions.TaskInvocationNotFoundError: when the task_inv_id
    ...
    def submit(
        ...
        Raises:
            orquestra.sdk._client.exceptions.DirtyGitRepo: if ``ignore_dirty_repo`` is False and
    ...
    def stop(
        ...
        Raises:
            orquestra.sdk._client.exceptions.UnauthorizedError: when communication with runtime
                failed because of an auth error
            orquestra.sdk._client.exceptions.WorkflowRunCanNotBeTerminated: if the termination
                attempt failed
        """
        wf_run = sdk.WorkflowRun.by_id(wf_run_id, config_name)
    ...
    def get_wf_outputs(self, wf_run_id: WorkflowRunId, config_name: ConfigName):
        ...
        Raises:
            orquestra.sdk._client.exceptions.NotFoundError: when the run_id doesn't match a
                stored run ID.
            orquestra.sdk._client.exceptions.ConfigNameNotFoundError: when the named config is
                not found in the file.
            orquestra.sdk._client.exceptions.WorkflowRunNotSucceeded: when the workflow is no
    ...
    def get_task_outputs(
        ...
        Raises:
            orquestra.sdk._client.exceptions.NotFoundError: when the wf_run_id doesn't match a
                known run ID.
            orquestra.sdk._client.exceptions.TaskInvocationNotFoundError: when task_inv_id
...
class ConfigRepo:
    ...
    def store_token_in_config(self, uri: str, token: str, runtime_name: RemoteRuntime):
        ...
        Raises:
            orquestra.sdk._client.exceptions.ExpiredTokenError: if the current date is after the
                token's expiry
            orquestra.sdk._client.exceptions.InvalidTokenError: if the token is not a JWT
        """
        try:
...
class WorkflowDefRepo:
    ...
    def get_worklow_names(self, module: ModuleType) -> t.Sequence[str]:
        ...
        Raises:
            orquestra.sdk._client.exceptions.NoWorkflowDefinitionsFound: when there were no
    ...
    def get_workflow_def(self, module: ModuleType, name: str) -> sdk.WorkflowDef:
        ...
        Raises:
            orquestra.sdk._client.exceptions.WorkflowSyntaxError: when the workflow of choice is
                parametrized.
        """
