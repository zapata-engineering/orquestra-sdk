...
from .exceptions import WorkflowSyntaxError
from .. import secrets
from . import _api, _dsl, _loader as loader
...
class WorkflowDef(Generic[_R]):
    ...
    def model(self) -> ir.WorkflowDef:
        ...
        Raises:
            orquestra.sdk._client.exceptions.DirtyGitRepo: (warning) when a task def used by
                this workflow def has a "GitImport" and the git repo that contains it
                has uncommitted changes.
            orquestra.sdk._client.exceptions.WorkflowSyntaxError: when there are no tasks
                defined for this workflow.
            orquestra.sdk._client.exceptions.InvalidTaskDefinitionError: when there is invalid
                task used inside this workflow
        """
        from orquestra.sdk._client import _traversal
    ...
    def graph(self):
        ...
        import orquestra.sdk._client._viz

        return orquestra.sdk._client._viz.wf_def_to_graphviz(self.model)
    ...
    def run(
        ...
        Raises:
            orquestra.sdk._client.exceptions.DirtyGitRepo: (warning) when a task def used by
                this workflow def has a "GitImport" and the git repo that contains it
                has uncommitted changes.
            orquestra.sdk._client.exceptions.ProjectInvalidError: when only 1 out of project and
                workspace is passed.
        """
        if project_dir:
            warnings.warn(
...
class WorkflowTemplate(Generic[_P, _R]):
    ...
    def __call__(self, *args: _P.args, **kwargs: _P.kwargs) -> WorkflowDef[_R]:
        ...
        Raises:
            ...
            orquestra.sdk._client.exceptions.WorkflowSyntaxError: if the arguments don't match
