...
from orquestra.sdk.schema.paths import ORQUESTRA_BASE_PATH
from .. import _env
from .._config import read_config
from .._env import CURRENT_USER_ENV
from .._jwt import get_email_from_jwt_token
from .._spaces._api import (
    make_workspace_url,
    make_workspace_zri,
)
from ..exceptions import ConfigNameNotFoundError, RuntimeConfigError
...
def get_current_user(config_name: t.Optional[str]) -> str:
    ...
    Raises:
        orquestra.sdk._client.exceptions.ConfigNameNotFoundError: when no matching config was
            found or when called locally without config parameter
        orquestra.sdk._client.exceptions.RuntimeConfigError: when chosen config does not
            contain token runtime option
        orquestra.sdk._client.exceptions.InvalidTokenError: When token saved inside config
