...
from orquestra.sdk.schema import serde
from orquestra.sdk.schema import exec_ctx
from orquestra.sdk.schema.logs import LogOutput
...
class TaskRun:
    ...
    def get_outputs(self) -> t.Any:
        ...
        return _serde.deserialize(task_outputs)
    ...
    def _find_value_by_id(
        ...
        if arg_id in self._wf_def.constant_nodes:
            value = serde.deserialize_constant(self._wf_def.constant_nodes[arg_id])
            return value
        ...
        parent_output_vals = _serde.deserialize(artifact_node)
...
def _get_ray_backend_ids() -> CurrentRunIDs:
    ...
    import orquestra.sdk.ray_wrapper._build_workflow

    (
        wf_run_id,
        task_inv_id,
        task_run_id,
    ) = orquestra.sdk.ray_wrapper._build_workflow.get_current_ids()
...
def _get_in_process_backend_ids() -> CurrentRunIDs:
    ...
    """
    from ..._client._in_process_runtime import get_current_in_process_ids
...
def current_run_ids() -> CurrentRunIDs:
    ...
    """
    context = exec_ctx.get_current_exec_context()

    if context == exec_ctx.ExecContext.RAY:
        return _get_ray_backend_ids()
    elif context == exec_ctx.ExecContext.DIRECT:
