...
from orquestra.sdk._client import exceptions
...
def build_runtime_from_config(
    ...
    if config.runtime_name == RuntimeName.RAY_LOCAL:
        import orquestra.sdk.ray_wrapper._dag

        return orquestra.sdk.ray_wrapper._dag.RayRuntime(
            config=config,
        )
    elif config.runtime_name == RuntimeName.IN_PROCESS:
        import orquestra.sdk._client._in_process_runtime

        return orquestra.sdk._client._in_process_runtime.InProcessRuntime()
...
def _build_ce_runtime(config: RuntimeConfiguration, verbose: bool):
    import orquestra.sdk._client._driver._ce_runtime
    import orquestra.sdk._client._driver._client
    ...
    uri_provider = orquestra.sdk._client._driver._client.ExternalUriProvider(base_uri)

    client = orquestra.sdk._client._driver._client.DriverClient.from_token(
        token=token, uri_provider=uri_provider
    )

    return orquestra.sdk._client._driver._ce_runtime.CERuntime(
        config=config,
        client=client,
        verbose=verbose,
    )
